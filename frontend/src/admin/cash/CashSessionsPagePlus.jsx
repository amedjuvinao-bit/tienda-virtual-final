// frontend/src/admin/cash/CashSessionsPagePlus.jsx
...
function movementAmountText(movement) {
  ...
  return `${sign}${money(movement?.amount)}`;
}
...
function MovementsBox({ session, form, setForm, disabled, onSubmit }) {
  const movements = Array.isArray(session?.cashMovements) ? [...session.cashMovements].reverse().slice(0, 8) : [];
...
export default function CashSessionsPagePlus() {
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm" style={{ color: 'var(--admin-card-muted-text)' }}>Controla apertura, ventas POS, movimientos manuales y cierre de caja por sede.</p>
            </div>
          </div>
