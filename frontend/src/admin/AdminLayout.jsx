...
export default function AdminLayout() {
  ...
  const formattedHeaderTime = currentTime.toLocaleTimeString('es-CO', {
    hour: 'numeric',
    minute: '2-digit',
  });
  const formattedHeaderDate = currentTime.toLocaleDateString('es-CO', {
    day: '2-digit',
    month: 'short',
  });
    ...
    <>
      <style>{`
        ...
        .admin-premium-nav-icon__halo {
          ...
          z-index: 0;
          border-radius: 10px;
          background: color-mix(in srgb, var(--admin-primary) 24%, transparent);
          filter: blur(9px);
          opacity: .22;
          transform: scale(.82);
        ...
        .admin-premium-nav-icon__tint {
          ...
          z-index: 2;
          border-radius: 50%;
          background: radial-gradient(circle, color-mix(in srgb, var(--admin-primary) 30%, rgba(255,255,255,.2)) 0%, transparent 74%);
          filter: blur(5px);
          opacity: .32;
          transform: scale(.86);
        ...
        .admin-premium-nav-icon__glyph {
          ...
          width: 24px;
          height: 24px;
          color: color-mix(in srgb, var(--admin-primary) 46%, #fff 54%);
          stroke-linecap: round;
          stroke-linejoin: round;
          filter:
            drop-shadow(0 .5px 0 rgba(255,255,255,.7))
            drop-shadow(0 1.5px 2px color-mix(in srgb, var(--admin-card-text) 16%, transparent));
        ...
        .admin-nav-link-mobile[aria-current="page"] .admin-premium-nav-icon__glyph {
          color: color-mix(in srgb, var(--admin-primary) 58%, #fff 42%);
          filter:
            drop-shadow(0 .5px 0 rgba(255,255,255,.8))
            drop-shadow(0 2px 3px color-mix(in srgb, var(--admin-primary) 20%, transparent));
        }

        .admin-nav-link:hover .admin-premium-nav-icon__glass,
        .admin-nav-link-mobile:hover .admin-premium-nav-icon__glass {
          border-color: color-mix(in srgb, var(--admin-primary) 28%, #fff 72%);
          box-shadow:
        ...
        @keyframes adminCrystalTintHover {
          ...
          52% { opacity: .62; transform: scale(1.08); }
          100% { opacity: .46; transform: scale(.98); }
        }

        @keyframes adminCrystalHaloHover {
          0% { opacity: .22; transform: scale(.82); filter: blur(9px); }
          52% { opacity: .58; transform: scale(1.08); filter: blur(10px); }
          100% { opacity: .38; transform: scale(.96); filter: blur(9px); }
        }
        ...
        .admin-command-result {
          ...
          gap: 9px;
          width: 100%;
          border: 1px solid color-mix(in srgb, var(--admin-card-border) 72%, transparent);
          border-radius: calc(var(--admin-radius) * .48);
          padding: 7px;
          background: color-mix(in srgb, var(--admin-page-bg) 94%, var(--admin-primary) 6%);
          color: inherit;
          text-align: left;
          cursor: pointer;
        }
        ...
        .admin-command-result:focus-visible {
          ...
          background: color-mix(in srgb, var(--admin-page-bg) 82%, var(--admin-primary) 18%);
          outline: none;
        }
          ...
          <main className="admin-main-column flex-1 min-w-0 flex flex-col">
            ...
            >
              ...
              <div className="admin-header-actions">
                <div
                  className="admin-header-status"
                  title={`${formattedHeaderDate} · ${formattedHeaderTime} · Sesión activa`}
                >
                  <div className="admin-header-status__icon">
                    <Clock3 className="h-4 w-4" />
                  </div>
                  <div>
                    <strong>{formattedHeaderTime}</strong>
                    <small>{formattedHeaderDate} · sesión activa</small>
                  </div>
                </div>
