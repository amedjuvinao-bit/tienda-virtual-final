...
function billingStepWasPersisted(
  stepId,
  requestedBilling = {},
  persistedSettings = {}
) {
...
export default function FacturacionSection() {
  ...
  const handleFinish = async () => {
    const stepResults = STEPS.slice(0, -1).map((item, index) => ({
      ...item,
      index,
  ...
  const step = STEPS[currentStep];
  const finalStepResults = STEPS.slice(0, -1).map((item, index) => ({
    ...item,
    index,
    errors: getBillingStepErrors(item.id, validationContext),
  }));
