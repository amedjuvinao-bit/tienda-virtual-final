// frontend/src/admin/customers/AdminCustomersPageEditable.jsx
    ...
    <div className="fixed inset-0 z-[130] flex items-start justify-center overflow-y-auto bg-slate-950/60 px-4 py-8 backdrop-blur-sm">
      <section className="w-full max-w-5xl overflow-hidden rounded-[30px] border" style={{ borderColor: 'rgba(236,72,153,0.28)', background: 'linear-gradient(145deg, #fff, #fff1f7)', boxShadow: '0 30px 90px rgba(15,23,42,0.28)' }}>
        <div className="border-b p-5 lg:p-6" style={{ borderColor: 'rgba(236,72,153,0.20)' }}>
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              {customer?.id ? <button type="button" onClick={() => setEditing((prev) => !prev)} className="inline-flex items-center gap-2 rounded-2xl border px-4 py-3 text-xs font-black" style={{ borderColor: 'rgba(236,72,153,0.22)', color: 'var(--admin-primary)', background: '#fff' }}><Edit3 className="h-4 w-4" /> {editing ? 'Cancelar edición' : 'Editar cliente'}</button> : null}
        ...
        <div className="space-y-5 p-5 lg:p-6">
          ...
          {editError ? <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm font-bold text-red-700">{editError}</div> : null}
          {editSuccess ? <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-bold text-emerald-700">{editSuccess}</div> : null}
          ...
          {customer && !loading ? (
            <>
              ...
              {editing ? (
                <form onSubmit={saveChanges} className="rounded-3xl border p-5" style={{ borderColor: 'rgba(236,72,153,0.18)', background: '#fff' }}>
                  <div className="mb-4 flex items-center justify-between gap-3">
                    <div>
                      <h3 className="text-sm font-black uppercase tracking-[0.16em]" style={{ color: 'var(--admin-card-muted-text)' }}>Editar cliente</h3>
                      <p className="mt-1 text-sm font-bold" style={{ color: 'var(--admin-card-muted-text)' }}>Guarda los cambios sin salir del detalle.</p>
                    </div>
                    <button type="submit" disabled={saving} className="inline-flex items-center gap-2 rounded-2xl px-4 py-3 text-xs font-black text-white disabled:opacity-60" style={{ background: 'var(--admin-primary)' }}>
                  ...
                  <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                    ...
                    <Field label="Tipo doc."><select value={editForm.documentType} onChange={(event) => updateField('documentType', event.target.value)} className="w-full rounded-2xl border px-4 py-3 text-sm font-bold outline-none" style={{ borderColor: 'rgba(236,72,153,0.28)', background: 'rgba(255,255,255,0.86)', color: 'var(--admin-card-text)' }}><option value="CC">CC</option><option value="CE">CE</option><option value="TI">TI</option><option value="NIT">NIT</option><option value="PP">Pasaporte</option><option value="OTHER">Otro</option></select></Field>
                  </div>
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                ...
                <div className="rounded-3xl border p-5" style={{ borderColor: 'rgba(236,72,153,0.18)', background: '#fff' }}>
                  ...
                  <div className="flex flex-wrap gap-2"><Badge tone="primary">{sourceLabel(customer.source)}</Badge><Badge tone={customer.status === 'active' ? 'success' : 'muted'}>{customer.status || 'active'}</Badge>{customer.acceptsMarketing ? <Badge tone="success">Acepta marketing</Badge> : <Badge tone="muted">Sin marketing</Badge>}</div>
              ...
              <div className="overflow-hidden rounded-3xl border" style={{ borderColor: 'rgba(236,72,153,0.18)', background: '#fff' }}>
                <div className="border-b px-5 py-4" style={{ borderColor: 'rgba(236,72,153,0.16)' }}><h3 className="text-sm font-black uppercase tracking-[0.16em]" style={{ color: 'var(--admin-card-muted-text)' }}>Historial reciente de compras</h3></div>
                {orders.length === 0 ? <p className="p-5 text-sm font-bold" style={{ color: 'var(--admin-card-muted-text)' }}>No se encontraron órdenes asociadas a este cliente.</p> : <div className="divide-y" style={{ borderColor: 'rgba(236,72,153,0.14)' }}>{orders.map((order) => <div key={order.id} className="grid gap-3 p-5 lg:grid-cols-[180px_1fr_150px_120px] lg:items-center"><div><p className="font-black" style={{ color: 'var(--admin-card-text)' }}>Orden {order.orderNumber}</p><p className="mt-1 text-xs font-bold" style={{ color: 'var(--admin-card-muted-text)' }}>{formatDate(order.createdAt)} · {sourceLabel(order.source)}</p></div><div>{(order.items || []).slice(0, 2).map((item, index) => <p key={`${order.id}-${index}`} className="text-xs font-bold" style={{ color: 'var(--admin-card-muted-text)' }}>{item.title} · {item.quantity} und. {item.size || item.color ? `· ${item.size || ''} ${item.color || ''}` : ''}</p>)}</div><p className="text-sm font-black" style={{ color: 'var(--admin-primary)' }}>{money(order.total)}</p><Badge tone={order.status === 'paid' ? 'success' : 'muted'}>{order.status || 'orden'}</Badge></div>)}</div>}
              </div>
            </>
          ) : null}
        </div>
...
export default function AdminCustomersPageEditable() {
    ...
    <div className="min-h-full space-y-5">
      ...
      <Card className="overflow-hidden p-5 lg:p-6" style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.98), rgba(252,231,243,0.86))' }}>
        <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex items-start gap-4"><IconBox icon={UserRound} /><div><div className="mb-2 inline-flex items-center gap-2 rounded-2xl border px-3 py-1.5 text-xs font-black uppercase tracking-[0.14em]" style={{ borderColor: 'rgba(236,72,153,0.22)', background: 'rgba(255,255,255,0.78)', color: 'var(--admin-primary)' }}><Sparkles className="h-3.5 w-3.5" /> CRM de clientes</div><h1 className="text-2xl font-black tracking-tight lg:text-3xl" style={{ color: 'var(--admin-card-text)' }}>Clientes</h1><p className="mt-1 max-w-2xl text-sm leading-relaxed" style={{ color: 'var(--admin-card-muted-text)' }}>Administra clientes de POS y tienda web con historial comercial, compras acumuladas y segmentación rápida.</p></div></div>
