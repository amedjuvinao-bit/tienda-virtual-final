// frontend/src/admin/customers/AdminCustomersPageCRM.jsx
...
function Field({ label, children }) {
  return (
    <label className="block">
      <span className="mb-2 block text-xs font-black uppercase tracking-[0.18em]" style={{ color: 'var(--admin-card-muted-text)' }}>{label}</span>
      {children}
    </label>
  );
}
...
function TextInput(props) {
  return (
    <input
      ...
      style={{ borderColor: 'rgba(236, 72, 153, 0.28)', background: 'rgba(255,255,255,0.86)', color: 'var(--admin-card-text)', ...(props.style || {}) }}
    />
  );
}
  ...
  return (
    <div
      ...
      style={{
        ...
        background: highlight ? 'linear-gradient(135deg, rgba(236,72,153,0.14), rgba(255,255,255,0.92))' : 'rgba(255,255,255,0.74)',
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-3 text-2xl font-black leading-tight" style={{ color: highlight ? 'var(--admin-primary)' : 'var(--admin-card-text)' }}>{value}</p>
          {helper ? <p className="mt-3 text-xs font-bold leading-relaxed" style={{ color: 'var(--admin-card-muted-text)' }}>{helper}</p> : null}
        </div>
        <IconBox icon={Icon} />
      </div>
    </div>
...
function CustomerRow({ customer, onOpenDetail }) {
  ...
  return (
    <article className="group overflow-hidden rounded-[28px] border p-4 transition duration-200 hover:-translate-y-0.5 lg:p-5" style={{ borderColor: 'rgba(236,72,153,0.20)', background: 'linear-gradient(135deg, rgba(255,255,255,0.96), rgba(253,242,248,0.74))', boxShadow: '0 14px 34px rgba(190, 24, 93, 0.08)' }}>
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.15fr)_minmax(260px,0.9fr)_minmax(310px,0.95fr)_120px] xl:items-center">
        <div className="min-w-0">
        ...
        <div className="rounded-3xl border p-4" style={{ borderColor: 'rgba(236,72,153,0.16)', background: 'rgba(255,255,255,0.68)' }}>
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="rounded-3xl border p-4 text-center" style={{ borderColor: 'rgba(236,72,153,0.18)', background: '#fff' }}><p className="text-[11px] font-black uppercase tracking-[0.12em]" style={{ color: 'var(--admin-card-muted-text)' }}>Compras</p><p className="mt-2 text-2xl font-black" style={{ color: 'var(--admin-card-text)' }}>{ordersCount}</p></div>
          <div className="rounded-3xl border p-4 text-center" style={{ borderColor: 'rgba(236,72,153,0.18)', background: '#fff' }}><p className="text-[11px] font-black uppercase tracking-[0.12em]" style={{ color: 'var(--admin-card-muted-text)' }}>POS</p><p className="mt-2 text-2xl font-black" style={{ color: 'var(--admin-card-text)' }}>{Number(stats.posOrdersCount || 0)}</p></div>
          <div className="rounded-3xl border p-4 text-center" style={{ borderColor: 'rgba(236,72,153,0.24)', background: '#fff7fb' }}><p className="text-[11px] font-black uppercase tracking-[0.12em]" style={{ color: 'var(--admin-card-muted-text)' }}>Gastado</p><p className="mt-2 break-words text-sm font-black leading-tight" style={{ color: 'var(--admin-primary)' }}>{money(totalSpent)}</p></div>
        </div>

        <button type="button" onClick={() => onOpenDetail(customer)} className="inline-flex items-center justify-center gap-2 rounded-2xl border px-4 py-3 text-xs font-black transition hover:-translate-y-0.5" style={{ borderColor: 'rgba(236,72,153,0.28)', background: '#fff', color: 'var(--admin-primary)' }}>
...
function SegmentPanel({ title, children }) {
  return <div className="rounded-[24px] border p-4" style={{ borderColor: 'rgba(236,72,153,0.16)', background: 'rgba(255,255,255,0.66)' }}><p className="mb-3 text-xs font-black uppercase tracking-[0.18em]" style={{ color: 'var(--admin-card-muted-text)' }}>{title}</p><div className="flex flex-wrap gap-2">{children}</div></div>;
}

function DetailMetric({ label, value, helper }) {
  return <div className="rounded-3xl border p-4" style={{ borderColor: 'rgba(236,72,153,0.18)', background: '#fff' }}><p className="text-xs font-black uppercase tracking-[0.14em]" style={{ color: 'var(--admin-card-muted-text)' }}>{label}</p><p className="mt-2 text-xl font-black" style={{ color: 'var(--admin-card-text)' }}>{value}</p>{helper ? <p className="mt-1 text-xs font-bold" style={{ color: 'var(--admin-card-muted-text)' }}>{helper}</p> : null}</div>;
}

function CustomerDetailModal({ data, loading, error, onClose, onRefresh }) {
  const customer = data?.customer || null;
  const orders = Array.isArray(data?.recentOrders) ? data.recentOrders : [];
  const stats = customer?.stats || {};

  return (
    <div className="fixed inset-0 z-[130] flex items-start justify-center overflow-y-auto bg-slate-950/60 px-4 py-8 backdrop-blur-sm">
      <section className="w-full max-w-5xl overflow-hidden rounded-[30px] border" style={{ borderColor: 'rgba(236,72,153,0.28)', background: 'linear-gradient(145deg, #fff, #fff1f7)', boxShadow: '0 30px 90px rgba(15,23,42,0.28)' }}>
        <div className="border-b p-5 lg:p-6" style={{ borderColor: 'rgba(236,72,153,0.20)' }}>
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex items-start gap-4">
              <IconBox icon={UserRound} />
              <div>
                <p className="text-xs font-black uppercase tracking-[0.18em]" style={{ color: 'var(--admin-primary)' }}>Detalle comercial</p>
                <h2 className="mt-1 text-2xl font-black" style={{ color: 'var(--admin-card-text)' }}>{customer ? customerName(customer) : 'Cliente'}</h2>
                <p className="mt-1 text-sm font-bold" style={{ color: 'var(--admin-card-muted-text)' }}>{customer?.customerCode || 'Consultando información...'}</p>
              </div>
            </div>
            <div className="flex flex-wrap gap-2">
              {customer?.id ? <button type="button" onClick={() => onRefresh(customer.id)} className="rounded-2xl border px-4 py-3 text-xs font-black" style={{ borderColor: 'rgba(236,72,153,0.22)', color: 'var(--admin-primary)', background: '#fff' }}>Actualizar</button> : null}
        ...
        <div className="space-y-5 p-5 lg:p-6">
          {loading ? <div className="rounded-3xl border p-8 text-center" style={{ borderColor: 'rgba(236,72,153,0.18)', background: '#fff' }}><Loader2 className="mx-auto h-8 w-8 animate-spin" style={{ color: 'var(--admin-primary)' }} /><p className="mt-3 font-black">Cargando detalle...</p></div> : null}
          ...
          {customer && !loading ? (
            <>
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <DetailMetric label="Compras" value={Number(stats.ordersCount || 0)} helper={`POS: ${Number(stats.posOrdersCount || 0)} · Web: ${Number(stats.webOrdersCount || 0)}`} />
                <DetailMetric label="Total gastado" value={money(stats.totalSpent)} helper={`Última orden: ${stats.lastOrderNumber || 'Sin registro'}`} />
                <DetailMetric label="Primera compra" value={stats.firstPurchaseAt ? formatDate(stats.firstPurchaseAt) : 'Sin registro'} />
                <DetailMetric label="Última compra" value={stats.lastPurchaseAt ? formatDate(stats.lastPurchaseAt) : 'Sin registro'} />
              </div>

              <div className="grid gap-4 lg:grid-cols-2">
                <div className="rounded-3xl border p-5" style={{ borderColor: 'rgba(236,72,153,0.18)', background: '#fff' }}>
                  ...
                  <div className="space-y-3 text-sm font-bold" style={{ color: 'var(--admin-card-text)' }}>
                    <p className="flex items-center gap-2"><Phone className="h-4 w-4" style={{ color: 'var(--admin-primary)' }} /> {customer.phone || 'Sin celular'}</p>
                    <p className="flex items-center gap-2"><Mail className="h-4 w-4" style={{ color: 'var(--admin-primary)' }} /> {customer.email || 'Sin correo'}</p>
                    <p className="flex items-center gap-2"><FileText className="h-4 w-4" style={{ color: 'var(--admin-primary)' }} /> {customer.documentType || 'DOC'} {customer.documentNumber || 'Sin documento'}</p>
                    <p className="flex items-center gap-2"><MapPin className="h-4 w-4" style={{ color: 'var(--admin-primary)' }} /> {customer.address || 'Sin dirección'} {customer.city ? `· ${customer.city}` : ''}</p>
                  </div>
                </div>
                <div className="rounded-3xl border p-5" style={{ borderColor: 'rgba(236,72,153,0.18)', background: '#fff' }}>
                  ...
                  <p className="mt-4 rounded-2xl border p-4 text-sm font-bold" style={{ borderColor: 'rgba(236,72,153,0.16)', color: 'var(--admin-card-muted-text)', background: '#fff7fb' }}>{customer.notes || 'Sin notas internas.'}</p>
                </div>
              </div>

              <div className="overflow-hidden rounded-3xl border" style={{ borderColor: 'rgba(236,72,153,0.18)', background: '#fff' }}>
                <div className="border-b px-5 py-4" style={{ borderColor: 'rgba(236,72,153,0.16)' }}>
                  <h3 className="text-sm font-black uppercase tracking-[0.16em]" style={{ color: 'var(--admin-card-muted-text)' }}>Historial reciente de compras</h3>
                </div>
                {orders.length === 0 ? <p className="p-5 text-sm font-bold" style={{ color: 'var(--admin-card-muted-text)' }}>No se encontraron órdenes asociadas a este cliente.</p> : (
                  <div className="divide-y" style={{ borderColor: 'rgba(236,72,153,0.14)' }}>
                    {orders.map((order) => (
                      <div key={order.id} className="grid gap-3 p-5 lg:grid-cols-[180px_1fr_150px_120px] lg:items-center">
                        <div><p className="font-black" style={{ color: 'var(--admin-card-text)' }}>Orden {order.orderNumber}</p><p className="mt-1 text-xs font-bold" style={{ color: 'var(--admin-card-muted-text)' }}>{formatDate(order.createdAt)} · {sourceLabel(order.source)}</p></div>
                        <div>{(order.items || []).slice(0, 2).map((item, index) => <p key={`${order.id}-${index}`} className="text-xs font-bold" style={{ color: 'var(--admin-card-muted-text)' }}>{item.title} · {item.quantity} und. {item.size || item.color ? `· ${item.size || ''} ${item.color || ''}` : ''}</p>)}</div>
                        <p className="text-sm font-black" style={{ color: 'var(--admin-primary)' }}>{money(order.total)}</p>
                        <Badge tone={order.status === 'paid' ? 'success' : 'muted'}>{order.status || 'orden'}</Badge>
                      </div>
                    ))}
                  </div>
                )}
...
export default function AdminCustomersPageCRM() {
  ...
  return (
    <div className="min-h-full space-y-5">
      {detailData ? <CustomerDetailModal data={detailData} loading={detailLoading} error={detailError} onClose={() => setDetailData(null)} onRefresh={(customerId) => openCustomerDetail({ id: customerId })} /> : null}
      ...
      <Card className="overflow-hidden p-5 lg:p-6" style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.98), rgba(252,231,243,0.86))' }}>
        <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex items-start gap-4">
            <IconBox icon={UserRound} />
            <div>
              <div className="mb-2 inline-flex items-center gap-2 rounded-2xl border px-3 py-1.5 text-xs font-black uppercase tracking-[0.14em]" style={{ borderColor: 'rgba(236,72,153,0.22)', background: 'rgba(255,255,255,0.78)', color: 'var(--admin-primary)' }}><Sparkles className="h-3.5 w-3.5" /> CRM de clientes</div>
              <h1 className="text-2xl font-black tracking-tight lg:text-3xl" style={{ color: 'var(--admin-card-text)' }}>Clientes</h1>
              <p className="mt-1 max-w-2xl text-sm leading-relaxed" style={{ color: 'var(--admin-card-muted-text)' }}>Administra clientes de POS y tienda web con historial comercial, compras acumuladas y segmentación rápida.</p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => loadCustomers(searchTerm, sourceFilter, segmentFilter)} disabled={loading} className="inline-flex items-center justify-center gap-2 rounded-2xl border px-4 py-3 text-sm font-black transition disabled:cursor-not-allowed disabled:opacity-60" style={{ borderColor: 'rgba(236,72,153,0.24)', background: '#fff', color: 'var(--admin-primary)' }}>{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />} Actualizar</button>
            <button type="button" onClick={() => setShowForm((prev) => !prev)} className="inline-flex items-center justify-center gap-2 rounded-2xl px-5 py-3 text-sm font-black text-white shadow-lg transition hover:-translate-y-0.5" style={{ background: 'linear-gradient(135deg, var(--admin-primary), #be185d)', boxShadow: '0 18px 36px rgba(236,72,153,0.28)' }}><Plus className="h-4 w-4" /> Nuevo cliente</button>
          </div>
        </div>
      </Card>
      ...
      {showForm ? (
        <Card className="p-5">
          <form className="space-y-4" onSubmit={handleCreateCustomer}>
            <div className="flex items-start justify-between gap-3"><div><h2 className="text-lg font-black" style={{ color: 'var(--admin-card-text)' }}>Crear cliente</h2><p className="mt-1 text-sm" style={{ color: 'var(--admin-card-muted-text)' }}>Este cliente quedará disponible para seleccionarlo en el POS.</p></div><button type="submit" disabled={!canSave} className="inline-flex items-center justify-center gap-2 rounded-2xl px-4 py-3 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-60" style={{ background: 'var(--admin-primary)' }}>{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Guardar</button></div>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              ...
              <Field label="Tipo documento"><select value={form.documentType} onChange={(event) => updateForm('documentType', event.target.value)} className="w-full rounded-2xl border px-4 py-3 text-sm font-bold outline-none" style={{ borderColor: 'rgba(236,72,153,0.28)', background: 'rgba(255,255,255,0.86)', color: 'var(--admin-card-text)' }}><option value="CC">CC</option><option value="CE">CE</option><option value="TI">TI</option><option value="NIT">NIT</option><option value="PP">Pasaporte</option><option value="OTHER">Otro</option></select></Field>
      ...
      <Card className="overflow-hidden">
        <div className="border-b p-5 lg:p-6" style={{ borderColor: 'rgba(236,72,153,0.18)' }}>
          <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(360px,0.7fr)] xl:items-start">
            <div><p className="text-xs font-black uppercase tracking-[0.18em]" style={{ color: 'var(--admin-card-muted-text)' }}>Segmentación</p><h2 className="mt-1 text-xl font-black" style={{ color: 'var(--admin-card-text)' }}>Listado de clientes</h2><p className="mt-1 text-sm" style={{ color: 'var(--admin-card-muted-text)' }}>{total} cliente(s) encontrado(s) · Origen: {activeSourceLabel} · Segmento: {activeSegmentLabel}</p></div>
            <div className="flex items-center gap-3 rounded-[22px] border px-4 py-3" style={{ borderColor: 'rgba(236,72,153,0.22)', background: 'rgba(255,255,255,0.78)' }}><Search className="h-5 w-5 shrink-0" style={{ color: 'var(--admin-primary)' }} /><input type="text" value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Buscar por nombre, celular, correo o documento" className="w-full bg-transparent text-sm font-bold outline-none" style={{ color: 'var(--admin-card-text)' }} />{loading ? <Loader2 className="h-5 w-5 animate-spin" style={{ color: 'var(--admin-primary)' }} /> : null}</div>
          </div>
          <div className="mt-5 grid gap-3 xl:grid-cols-2">
            <SegmentPanel title="Origen del cliente">{SOURCE_FILTERS.map((filter) => <FilterPill key={filter.key} active={sourceFilter === filter.key} onClick={() => setSourceFilter(filter.key)}>{filter.label}</FilterPill>)}</SegmentPanel>
            <SegmentPanel title="Segmento comercial">{SEGMENT_FILTERS.map((filter) => <FilterPill key={filter.key} active={segmentFilter === filter.key} onClick={() => setSegmentFilter(filter.key)}>{filter.label}</FilterPill>)}{(sourceFilter !== 'all' || segmentFilter !== 'all' || searchTerm) ? <FilterPill active={false} onClick={resetFilters}>Limpiar filtros</FilterPill> : null}</SegmentPanel>
          </div>
        </div>

        <div className="p-5 lg:p-6">
          {loading ? <div className="rounded-[28px] border p-10 text-center" style={{ borderColor: 'rgba(236,72,153,0.18)', background: 'rgba(255,255,255,0.68)' }}><Loader2 className="mx-auto h-8 w-8 animate-spin" style={{ color: 'var(--admin-primary)' }} /><p className="mt-3 text-sm font-black" style={{ color: 'var(--admin-card-text)' }}>Cargando clientes...</p></div> : customers.length === 0 ? <div className="rounded-[28px] border p-10 text-center" style={{ borderColor: 'rgba(236,72,153,0.18)', background: 'rgba(255,255,255,0.68)' }}><UserRound className="mx-auto h-9 w-9" style={{ color: 'var(--admin-primary)' }} /><p className="mt-3 text-sm font-black" style={{ color: 'var(--admin-card-text)' }}>No hay clientes para este filtro</p><p className="mx-auto mt-2 max-w-md text-sm" style={{ color: 'var(--admin-card-muted-text)' }}>Cambia el filtro o crea un cliente nuevo.</p></div> : <div className="space-y-4">{customers.map((customer) => <CustomerRow key={customer.id} customer={customer} onOpenDetail={openCustomerDetail} />)}</div>}
        </div>
      </Card>
    </div>
  );
