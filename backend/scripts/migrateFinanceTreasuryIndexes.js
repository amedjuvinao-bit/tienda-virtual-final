...
} = require('../models/financeTreasuryIndexDefinitions');
const {
  applyCanonicalIndexes,
} = require('../services/indexMigrations/canonicalIndexMigration');

const APPLY_FLAG = '--apply-finance-treasury-index-migration';
const PRODUCTION_CONFIRMATION_FLAG =
  '--confirm-production-finance-treasury-index-migration';
const CODE_PREFIX = 'FINANCE_TREASURY_INDEX_MIGRATION';

const COLLECTIONS = Object.freeze([
  Object.freeze({
    collection: 'financeexpenses',
    definitions: FINANCE_TREASURY_EXPENSE_INDEX_DEFINITIONS,
  }),
  Object.freeze({
    collection: 'orders',
    definitions: FINANCE_TREASURY_ORDER_INDEX_DEFINITIONS,
  }),
]);
...
function safeError(error) {
  return {
    ...
    message: error?.code
      ? error.message
      : 'No fue posible completar la migración de índices de tesorería.',
