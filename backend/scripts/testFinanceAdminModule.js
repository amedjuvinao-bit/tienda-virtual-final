...
async function validateReports(query) {
  ...
  assert(
    Number(sales.revenue || 0) === 335000,
    `Ventas no suman POS + Web correctamente. Actual: ${sales.revenue}; ordenes: ${sales.ordersCount}; rango: ${query.dateFrom} a ${query.dateTo}`
  );
  assert(Number(sales.ordersCount || 0) === 2, `Cantidad de ordenes financieras no coincide. Actual: ${sales.ordersCount}`);
  ...
  assert(Number(profit.cogs || 0) === 200000, `Costo de venta no coincide con costos de producto/variante. Actual: ${profit.cogs}`);
  ...
  assert(Number(cash.sessionsCount || 0) === 1, `Caja financiera no detecta la sesion temporal. Actual: ${cash.sessionsCount}`);
  assert(Number(cash.movements?.operatingExpenses || 0) === 10000, `Caja no suma gastos operativos de movimientos. Actual: ${cash.movements?.operatingExpenses}`);
  assert(Number(cash.paymentTotals?.cash || 0) === 200000, `Caja no suma pagos en efectivo. Actual: ${cash.paymentTotals?.cash}`);
  ok('Reporte de caja calcula ventas, efectivo y movimientos');

  const expenses = await financeService.getExpensesReport(query);
  assert(Number(expenses.manualTotal || 0) === 6000, `Gastos manuales no suman correctamente. Actual: ${expenses.manualTotal}`);
  ...
  assert(Number(summary.kpis.revenue || 0) === 335000, `Resumen financiero no trae ingresos correctos. Actual: ${summary.kpis.revenue}`);
  assert(Number(summary.kpis.cogs || 0) === 200000, `Resumen financiero no trae costos correctos. Actual: ${summary.kpis.cogs}`);
  assert(Number(summary.kpis.operatingExpenses || 0) === 16000, `Resumen financiero no suma gastos manuales + caja. Actual: ${summary.kpis.operatingExpenses}`);
  assert(Number(summary.kpis.netProfit || 0) === 119000, `Resumen financiero no calcula utilidad neta correcta. Actual: ${summary.kpis.netProfit}`);
