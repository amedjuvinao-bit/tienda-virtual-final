...
} = require('./wompiFactusSandboxTrace/saleShippingStage');
const {
...
const { getShippingProviderStatus } = require('../services/shippingProviderService');
...
async function run() {
  ...
  const [settings, payments, shipping] = await Promise.all([
  ...
  const envia = assertEnviaSandboxConfig(shipping);

  console.log('\nTRAZA AUTÓNOMA — WOMPI + FACTUS + ENVIA');
  console.log(`Wompi: ${baseUrl}`);
  console.log(`Factus: ${factus.apiUrl}`);
  console.log(`Envia: ${envia.webhookUrl}`);
  console.log('No almacena tarjeta y no contacta ambientes de producción.\n');
  console.log('Las confirmaciones físicas son QA; Picked Up y Delivered deben llegar desde Envia.\n');
  ...
  if (args.autonomous) {
    ...
    console.log(
      `OK 1/12: carrito firmado y orden ${orderNumber} creados con inventario real`
    );
    console.log(
      `OK 2/12: reserva ${checkout.order.inventoryControl.reservationId} e intento de pago persistidos`
    );
    ...
    console.log(`OK 3/12: Wompi creó y aprobó la transacción ${transactionId}`);
  } else {
  ...
  console.log(
    `${args.autonomous ? 'OK 4/12' : 'OK 1/9'}: Wompi confirmó ${verified.status} y la propiedad de la transacción`
  );

  const paidOrder = await applyVerifiedApproval({ ...verified, payments });
  console.log(
    `${args.autonomous ? 'OK 5/12' : 'OK 2/9'}: pago e inventario aplicados por el motor canónico`
  );
  ...
  console.log(
    `${args.autonomous ? 'OK 6/12' : 'OK 3/9'}: factura Factus ${invoice.invoiceNumber} validada con PDF/XML`
  );

  const delivered = await ensureSandboxSaleDelivered(paidOrder);
  console.log(
    `${args.autonomous ? 'OK 7/12' : 'OK 4/9'}: retiro de la venta confirmado por la logística canónica`
  );

  const rma = await completeSandboxRma(delivered.order);
  console.log(
    `${args.autonomous ? 'OK 8/12' : 'OK 5/9'}: RMA ${rma.returnCase.returnNumber} recogido, recibido e inspeccionado`
  );
  console.log(
    `${args.autonomous ? 'OK 9/12' : 'OK 6/9'}: reembolso ${rma.refund.refundNumber} creado después de la inspección`
  );

  const completed = await automateSandboxRefund({ order: rma.order, refund: rma.refund });
  const note = await verifyCreditNoteDocuments(completed.finalInvoice);
  console.log(
    `${args.autonomous ? 'OK 10/12' : 'OK 7/9'}: dinero e inventario conciliados por el motor canónico`
  );
  console.log(
    `${args.autonomous ? 'OK 11/12' : 'OK 8/9'}: nota crédito Factus ${note.provider.number} validada con PDF/XML`
  );
  ...
  console.log(
    `${args.autonomous ? 'OK 12/12' : 'OK 9/9'}: Wompi confirmó ${voided.status}; ${events} eventos auditables`
  );
  ...
  console.log(`RMA Envia: ${rma.returnCase.returnNumber}`);
...
run()
  .catch((error) => {
    console.error('\nFALLO TRAZA WOMPI + FACTUS + ENVIA SANDBOX:', error?.code || error?.message || error);
