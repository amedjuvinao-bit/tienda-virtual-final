...
5. Consultar **Ver actividad** si se poseen `admin-users:view` y `logs:view`. El historial muestra altas, cambios y eventos 2FA ligados a la cuenta, actor, fecha y resultado, en páginas de diez registros. El servidor comprueba el alcance por sede antes de leer auditoría y solo devuelve campos de presentación. Las nuevas altas registran el ID de la cuenta para que también aparezcan en el historial.
...
| Logs | La auditoría administrativa y los cambios 2FA alimentan el historial individual; requiere el permiso de Logs. |
