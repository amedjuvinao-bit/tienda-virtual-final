...
- La traza autónoma se ejecuta con `npm --prefix backend run demo:orders-wompi-factus-sandbox -- --confirm-persist --confirm-wompi-sandbox --confirm-factus-habilitacion`. Selecciona un producto físico visible con inventario elegible, crea un carrito firmado y una orden mediante el checkout oficial, genera el intento, cifra en memoria la tarjeta oficial aprobada de Sandbox con JWE, crea y confirma la transacción real en Wompi Sandbox, aplica el pago con el motor canónico, valida factura y documentos en Factus habilitación, crea el reembolso total, solicita el `void` y valida la nota crédito con PDF/XML. Exige llaves Wompi `test`, bloquea `NODE_ENV=production`, saldo a favor y cualquier URL externa de producción; no registra tarjeta, token o JWE y conserva toda la evidencia. Una ejecución interrumpida puede reanudarse con `--resume-order=<NUMERO_ORDEN> --wompi-transaction=<ID_WOMPI>` junto con las tres confirmaciones.
