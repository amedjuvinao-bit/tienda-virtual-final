...
- La traza externa de cancelación se ejecuta sobre una compra creada por el checkout oficial con `npm --prefix backend run demo:orders-wompi-factus-sandbox -- --order=<NUMERO_ORDEN> --wompi-transaction=<ID_WOMPI> --confirm-persist --confirm-wompi-sandbox --confirm-factus-habilitacion`. Verifica la transacción directamente en Wompi Sandbox, aplica el pago con el motor canónico, valida factura y documentos en Factus habilitación, crea un reembolso total anterior al despacho, solicita el `void` real y valida la nota crédito con PDF/XML. Exige llaves Wompi `test`, bloquea `NODE_ENV=production`, pagos mixtos, otros medios de pago y cualquier URL de producción. No recibe ni conserva datos de tarjeta y puede reintentarse sin duplicar pago, reembolso o documento fiscal.
