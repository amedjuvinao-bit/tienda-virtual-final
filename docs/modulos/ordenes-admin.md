...
- La traza autónoma integral se ejecuta con `npm --prefix backend run demo:orders-wompi-factus-envia-sandbox -- --confirm-persist --confirm-wompi-sandbox --confirm-factus-habilitacion --confirm-envia-sandbox`. Selecciona un producto físico visible con inventario elegible, crea el carrito, la orden y el intento mediante los flujos oficiales, cifra en memoria la tarjeta de prueba con JWE, confirma el pago real en Wompi Sandbox y valida la factura Factus con PDF/XML. La venta se cierra con una confirmación administrativa QA de retiro en tienda para conservar un reembolso total; después crea y autoriza el RMA, genera una guía real Envia Sandbox, programa una recolección confirmada por Envia, solicita los eventos oficiales `Picked Up` y `Delivered`, espera ambos webhooks y solo entonces registra las confirmaciones QA de recepción e inspección. Finalmente concilia inventario y dinero, solicita el `void` total y valida la nota crédito Factus con PDF/XML. No presenta como físicos los pasos QA ni altera estados para reemplazar los webhooks de Envia. Exige Wompi Sandbox, Factus habilitación, Envia Sandbox activo y un webhook HTTPS ya comprobado; bloquea `NODE_ENV=production`, no registra tarjeta, token o JWE y conserva toda la evidencia. Una ejecución interrumpida puede reanudarse con `--resume-order=<NUMERO_ORDEN> --wompi-transaction=<ID_WOMPI>` junto con las cuatro confirmaciones.
...
La fase está protegida por 21 verificaciones del adaptador de transportadoras, 10 verificaciones de configuración y seguridad, 11 pruebas del centro visual de Envíos y del panel de Envia, las pruebas del centro logístico y el build de producción. GitHub Actions ejecuta tanto los contratos del backend como `test:shipping-settings-ui`; ninguna de esas pruebas llama a Envia ni crea guías reales.
